...
#define FOREACH_YPASS(CData) for (int j = 0; j < CData.x_passes; UpdYPass(CData, j))
#define FOREACH_XPASS(CData) for (int i = 0; i < CData.y_passes; UpdXPass(CData, i))
...
void WritePassToBuff(FBuff* Buff, int i, int j, CycleData& CData, Color4& color) {
	int pos_x = i * CData.pxl_size - CData.pxl_size / 2 + 1;
	int pos_y = j * CData.pxl_size - CData.pxl_size / 2 + 1;
